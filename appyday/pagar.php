    ...
    <body>
        ...
        <div id="menu">
            ...
            if($_REQUEST["pedir"] == 1) {
                ...
                $pedidos = $connection->prepare($connection, "UPDATE Carrinho SET status = 'pagar' WHERE email = ?;");
                $pedidos->bind_param("s", $_SESSION["client"]);
                $pedidos->execute();
                $connection->close();
            }
            ?>
        <div id="main">
            ...
            <div class="content">
                ...
                <?php
                    ...
                    $pedidos = mysqli_query($connection, "SELECT DISTINCT nome, produtosQTD, preco FROM Carrinho NATURAL JOIN Produtos WHERE status = 'pagar' ORDER BY nome;");
                ...
                <form method="post" action="historico.php" class="pure-form pure-form-aligned">
                    <fieldset>
                        <button class="pure-button" onclick="history.go(-1);return false;">Voltar</button>
                        <input type="hidden" name="pagar" value="1">
                        <button type="submit" class="pure-button pure-button-primary">Pagar tudo</button>
                    </fieldset>
                </form>
            </div>
        </div>
